...
class SearchEvaluator(Evaluator.Evaluator):
    ...
    def evaluate(self, input, target_output, output, log):
        try:
            ...
            number_of_pillars = int(lines[1])
            first_item_index = 3+number_of_pillars
            for i in range(3,3+number_of_pillars):
                pillar_string = lines[i].split("\t")
                pillars.append((int(pillar_string[0]),int(pillar_string[1])))
            ...
            for index,pillar in enumerate(pillars):
                x,y = pillar
                object_ids_around_pillar = [M[x-1,y-1],M[x,y-1],M[x,y],M[x-1,y]]
                counter = Counter(object_ids_around_pillar) 
                if counter.most_common()[0][1] > 2:
                    return (0,"Pillar {0} intersecting object: object {1} is bordering a pillar on more than 2 sides for input:\n\n {2}".format(index,counter.most_common()[0][0],input))
            return (1, "")
